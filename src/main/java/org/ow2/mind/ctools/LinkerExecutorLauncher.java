...
public class LinkerExecutorLauncher {
	...
	public static final CmdAppendOption       inputFileOpt            = new CmdAppendOption(
			ID_PREFIX
			+ "InputFile",
			null, "input-file",
			"A file to link in","INPUT_FILE");
	...
	public static final CmdArgument       scriptOpt                = new CmdArgument(
			ID_PREFIX
			+ "Script",
			null, "script",
			"The linker script to use","LDSCRIPT",null,false);

	public static CommandLine cmdLine;
	
	public LinkerExecutor executor;
	/**
	 * @param args
	...
	public void run(String[] args) {
		...
		options.addOptions(executableOpt,helpOpt,inputFileOpt,outputFileOpt,flagsOpt,scriptOpt);
		try {
	...
	private static void printUsage(final PrintStream ps) {
		ps.println("Usage: " + getProgramName()
				+ inputFileOpt.getPrototype() + outputFileOpt.getPrototype() + " [OPTIONS]+");
